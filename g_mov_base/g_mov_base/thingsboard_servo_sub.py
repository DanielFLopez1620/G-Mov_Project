...
class ThingsboardAccelSubs(Node):
    """
    Class that subscriber to the servo angle topic, obtain the data and then
    publish it to the ThingSBoard platform where is located a MQTT broker and
    allow the option to display it in a dashboard.
    """
    def __init__(self):
        """
    ...
    def mqtt_callback(self, msg) -> None:
        """
        Callback that read the servo position, which is later send to
        ThingsBoard.
...
def main(args=None):
    """
    Program that receives info of the servo angle topic and sent it to the
    ...
    # Instance subscriber
    servo_to_mqtt = ThingsboardAccelSubs()
